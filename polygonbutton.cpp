...
PolygonButton::PolygonButton(const QString &text,
                             qreal centerX_a,
                             qreal centerY_a,
                             qreal radius,
                             qreal angle,
    ...
    , m_hoverBorderColor(Qt::white)
    , m_isHovered(false)
    , m_gradientColorStart(QColor(31, 31, 31, 120))
    , m_gradientColorMiddle(QColor(1, 1, 1, 250))
    , m_gradientColorEnd(QColor(0, 0, 0, 206))
    , m_gradientProgress(0.0)
    , m_size(0)
    , m_radius(0)
    , rad(radius)
    , fontSize(13)
...
void PolygonButton::updatePolygon()
{
    ...
    QPolygonF polygon;
    polygon << center
            << QPointF(m_centerX + ((rad + m_radius) * m_size) * cos(qDegreesToRadians(m_angle)),
                       m_centerY + ((rad + m_radius) * m_size) * sin(qDegreesToRadians(m_angle)))
            << QPointF(m_centerX + ((rad + m_radius) * m_size) * cos(qDegreesToRadians(m_nextangle)),
                       m_centerY
                           + ((rad + m_radius) * m_size) * sin(qDegreesToRadians(m_nextangle)));

    QRegion region(polygon.toPolygon());
    setMask(region);
    inscribedRect = Utils::CalculateRec(polygon);
    fontSize = calculateFontSize(Utils::getLongestWord(text()), inscribedRect, 15);
    update();
}
...
void PolygonButton::paintEvent(QPaintEvent *event) {
    ...
    QPointF center(m_centerX, m_centerY);
    QPointF p1(m_centerX + ((rad + m_radius) * m_size) * cos(qDegreesToRadians(m_angle)),
               m_centerY + ((rad + m_radius) * m_size) * sin(qDegreesToRadians(m_angle)));
    QPointF p2(m_centerX + ((rad + m_radius) * m_size) * cos(qDegreesToRadians(m_nextangle)),
               m_centerY + ((rad + m_radius) * m_size) * sin(qDegreesToRadians(m_nextangle)));
    QPolygonF polygon;
    ...
    transitionGradient.setColorAt(0.5, QColor::fromRgbF(
                                           m_gradientColorMiddle.redF() * m_gradientProgress + m_gradientColorStart.redF() * (1 - m_gradientProgress),
                                           m_gradientColorMiddle.greenF() * m_gradientProgress + m_gradientColorStart.greenF() * (1 - m_gradientProgress),
                                           m_gradientColorMiddle.blueF() * m_gradientProgress + m_gradientColorStart.blueF() * (1 - m_gradientProgress),
                                           m_gradientColorMiddle.alphaF() * m_gradientProgress + m_gradientColorStart.alphaF() * (1 - m_gradientProgress)));
    transitionGradient.setColorAt(0, QColor::fromRgbF(
                                         m_gradientColorEnd.redF() * m_gradientProgress + m_gradientColorMiddle.redF() * (1 - m_gradientProgress),
                                         m_gradientColorEnd.greenF() * m_gradientProgress + m_gradientColorMiddle.greenF() * (1 - m_gradientProgress),
                                         m_gradientColorEnd.blueF() * m_gradientProgress + m_gradientColorMiddle.blueF() * (1 - m_gradientProgress),
                                         m_gradientColorEnd.alphaF() * m_gradientProgress + m_gradientColorMiddle.alphaF() * (1 - m_gradientProgress)));
    ...
    font.setBold(true);
    font.setFamily("CaskaydiaCove Nerd Font"); // Cambia la familia de la fuente aquí
    font.setPointSize(fontSize);
    ...
    QPointF topLeft(inscribe.left(), inscribe.top() + (inscribe.height() - textHeight) / 2);

    // Configurar el texto
    QString htmlText = QString("<strong style=\"color: #ffffff\">%1</strong>").arg(displayText);
    textDoc.setHtml(htmlText);

    painter.translate(topLeft); // Mover el pintor a la esquina superior izquierda del rectángulo centrado
...
void PolygonButton::setText(const QString &text)
{
    m_text = text;
}

QString PolygonButton::text() const
{
    return m_text;
}
...
qreal PolygonButton::size() const
{
    return m_size;
}
...
qreal PolygonButton::radius() const{
    return m_radius;
}


qreal PolygonButton::centerX() const {
    return m_centerX;
}
...
qreal PolygonButton::centerY() const {
    return m_centerY;
}
...
QPolygonF PolygonButton::polygon() const {
    return m_polygon;
}
...
qreal PolygonButton::gradientProgress() const {
    return m_gradientProgress;
}
...
QColor PolygonButton::gradientColorEnd() const {
...
void PolygonButton::startSizeAnimation() {
