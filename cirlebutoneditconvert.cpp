...
CirleButonEditConvert::CirleButonEditConvert(const QString &text,
                                             qreal radius,
                                             qreal centerx,
                                             qreal centery,
                                             const QChar &icon,
    ...
    , m_elementsize(0)
{
    ...
    m_moving = new QPropertyAnimation(this, "moving"); // Animación sobre la propiedad 'pos'
    ...
    m_mogAnimation->setEndValue((radius * 3.5) / 2); // Levanta el ícono 10 píxeles hacia arriba
    ...
    placeholder->setStyleSheet(
        "color: gray; font-size: 25pt;background: transparent;"); // Tamaño del placeholder
    ...
    editline->setStyleSheet("QTextEdit { "
    ...
    //editline->show();
    //connect(this, &QPushButton::toggled, this, &CirleButonEditConvert::updateIcon);

    connect(editline, &QTextEdit::textChanged, this, &CirleButonEditConvert::updatePlaceholder);
    ...
    treants = new CircularButton("", fa::fa_layer_group, fa::fa_list_check, radius / 2, this);
    clear = new CircularButton("", fa::fa_circle_check, fa::fa_delete_left, radius / 2, this);
    ...
    //CircularButton *clear = new CircularButton("", fa::fa_circle_check, fa::fa_delete_left, 0, this);
    //CircularButton *sendmessage = new CircularButton("",fa::fa_circle_check,fa::fa_delete_left,0,this);

    // Crear un efecto de opacidad para el botón
    QGraphicsOpacityEffect *opacityEffectClear = new QGraphicsOpacityEffect(clear);
    ...
    m_opacityTreant->setStartValue(0.0); // Comienza con 0% de opacidad (invisible)
    m_opacityTreant->setEndValue(1.0);   // Termina con 100% de opacidad (totalmente visible)

    // Crear un efecto de opacidad para el botón
    QGraphicsOpacityEffect *opacityEffectTreant = new QGraphicsOpacityEffect(treants);
    ...
    m_opacityClear->setEndValue(1.0);   // Termina con 100% de opacidad (totalmente visible)
}
...
void CirleButonEditConvert::animatePosition(const QPoint &startPos, const QPoint &endPos)
{
...
QSize CirleButonEditConvert::scaleSize() const
{
    return size();
}
...
qreal CirleButonEditConvert::mog() const
{
    return m_pos;
}
...
qreal CirleButonEditConvert::pos() const
{
    return m_pos;
}
...
QPointF CirleButonEditConvert::moving()
{
    return mov;
}
...
qreal CirleButonEditConvert::opacity() const
{
    return m_opacity;
}
...
qreal CirleButonEditConvert::zoom() const
{
    return m_zoom;
}
...
qreal CirleButonEditConvert::elementexpancion()
{
...
qreal CirleButonEditConvert::expancion()
{
    return exp;
}
void CirleButonEditConvert::setExpancion(qreal number)
{
    ...
    updateMask();
    setGeometry(m_centerX - (radius * exp),
                m_centerY - (radius * exp),
                (radius * exp),
                (radius * exp));
    update();
}

void CirleButonEditConvert::setText(const QString &text)
{
    m_text = text;
}

QString CirleButonEditConvert::text() const
{
    return m_text;
}
