...
class VoiceRoulette : public QWidget
{
    ...
    Q_PROPERTY(double radiusButons READ radiusButons WRITE setradiusButons NOTIFY radiusButonsChanged)
...
private slots:
    ...
    void handleHover(int x, int y);  // Slot to handle the hover signal from CircularTriangleButton
    void handleLeave();              // Slot to handle the hover signal from CircularTriangleButton
};
