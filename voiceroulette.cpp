...
VoiceRoulette::VoiceRoulette(QWidget *parent)
  ...
  QStringList Musics = {
      ...
      "Culebra python 1 ",
      "Culebra python 1 ",
      "Culebra pythinscribedRectinscribedRectinscribedRectpython 1Culebra "
      "python 11"};
...
void VoiceRoulette::setupButtons(QStringList list) {
  ...
  for (int i = 0; i < list.size(); ++i) {
    ...
    // connect(button, &PolygonButton::clicked, this,
    // &VoiceRoulette::onButtonClicked);
...
void VoiceRoulette::setupButtonsMenu(QMap<QString, QPair<QChar, QChar>> list) {
  ...
  double borderOffset =
      10; // Ajusta este valor para cambiar el tamaño del borde
  ...
  for (auto it = list.begin(); it != list.end(); ++it, ++i) {
    ...
    // connect(button, &CircularTriangleButton::clicked, this,
    // &VoiceRoulette::onButtonClicked); connect(button,
    // &CircularTriangleButton::hoverEntered, this,
    // &VoiceRoulette::handleHover); connect(button,
    // &CircularTriangleButton::hoverLeave, this, &VoiceRoulette::handleLeave);
...
void VoiceRoulette::mouseMoveEvent(QMouseEvent *event) {
  ...
  if (m_menuSelect) {
    for (const ButtonDataMenu *data : m_buttonsMenu) {
      double start = data->startAngle + margin;
      double end = data->endAngle - margin;
...
void VoiceRoulette::startAnimations() {
  ...
  for (int i = 0; i < indices.size(); ++i) {
    int delay = QRandomGenerator::global()->bounded(
        0, 50); // Genera un retraso aleatorio entre 50 y 1000 milisegundos
