...
CircularTriangleButton::CircularTriangleButton(const QString &text,
                                               const QChar &icon,
...
{
    //setCheckable(true);
    //connect(this, &QPushButton::toggled, this, &CircularTriangleButton::updateIcon);
...
void CircularTriangleButton::paintEvent(QPaintEvent *event) {
    ...
    QFont font = awesome->font(fa::fa_solid, 35*(1+m_zoom)); // Establecer el tamaño de fuente 16
    ...
    // Calcular la posición superior izquierda del ícono para centrarlo en el centro del polígono
...
qreal CircularTriangleButton::zoom() const {
    return m_zoom;
}
...
qreal CircularTriangleButton::radius() const{
    return m_radius;
}


qreal CircularTriangleButton::centerX() const {
    return m_centerX;
}
...
qreal CircularTriangleButton::centerY() const {
    return m_centerY;
}
...
QPolygonF CircularTriangleButton::polygon() const {
    return m_polygon;
}
