...
CircularButton::CircularButton(const QString &text, const QChar &icon,
                               const QChar &iconCheck, qreal radius,
                               QWidget *parent)
    : QPushButton(text, parent), m_backgroundColor(Qt::black), exp(1),
      ...
      m_hoverBackgroundColor(QColor(60, 60, 60)) {
  setCheckable(true);
...
void CircularButton::paintEvent(QPaintEvent *event) {
  ...
  QFont font = awesome->font(fa::fa_solid,
                             1 + radius); // Establecer el tamaño de fuente 16
...
void CircularButton::enterEvent(QEnterEvent *event) { Q_UNUSED(event); }

void CircularButton::leaveEvent(QEvent *event) { Q_UNUSED(event); }

qreal CircularButton::expancion() { return exp; }
