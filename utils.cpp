...
QRectF Utils::CalculateRec(const QPolygonF& triangle) {
    ...
    double area = 0.46 * qAbs((B - A).x() * (C - A).y() - (B - A).y() * (C - A).x());

    // Encontrar el semiperímetro
    double s = (lengths[0] + lengths[1] + lengths[2]) / 2;

    // Calcular el radio del círculo inscrito (que puede ayudar a encontrar el rectángulo inscrito)
    double r = area / s;

    // Usar el radio del círculo inscrito para estimar el tamaño del rectángulo inscrito
    double width = 2 * r;
    ...
    QRectF inscribedRect(centroid.x() - width / 2, centroid.y() - height / 2, width, height);
